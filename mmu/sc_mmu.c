...
void mmu_init(void) {
	...
	for (t = 0; t < memory_count; t++) {
		m = MemGet(t);
		kernelPageCount = (m->userStartAddress - m->globalStartAddress)
				/ MMU_MASTER_TABLE_PAGE_SIZE;
		// if there is still a little space, reserve one more
		if (((m->userStartAddress - m->globalStartAddress)
				% MMU_MASTER_TABLE_PAGE_SIZE) > 0) {
			kernelPageCount++;
		}
...
Boolean mmu_handle_data_abort() {
	...
	if (currentProcess == NULL) {
		...
		if (mmu_is_legal(mmu_accessed_address, mmu_fault_state)) {
			...
			mmu_create_page_mapping(currentProcess->masterTable,
					mmu_accessed_address, 0);
			return TRUE;
		} else {
			printf(
					"illegal accessed address (0x%x) at position (0x%x) bye bye pid %i\n",
					mmu_accessed_address, 0x4711, currentProcess->id);
...
void mmu_create_page_mapping(tablePointer masterTable, uint32_t virtualAddress,
		uint8_t domain) {
	...
	mmu_create_address_mapping(masterTable, virtualAddress,
...
void mmu_create_address_mapping_range(tablePointer masterTable,
		uint32_t virtualStartAddress, uint32_t physicalStartAddress,
		uint32_t physicalEndAddress, uint8_t domain) {
	// create mappings in l2 table steps
	uint32_t i;
	uint32_t l2EntryCount;
	// mark page as reserved
	l2EntryCount = (physicalEndAddress - physicalStartAddress)
			/ MMU_L2_PAGE_SIZE;
	// if there is still a little space, reserve one more
	...
	for (i = 0; i < l2EntryCount; i++) {
		mmu_create_address_mapping(masterTable,
				...
				physicalStartAddress + (i * MMU_L2_PAGE_SIZE), domain);

	}
}

void mmu_create_address_mapping(tablePointer masterTable,
		uint32_t virtualAddress, uint32_t physicalAddress, uint8_t domain) {
	...
	l2Table = mmu_get_or_create_l2_table(masterTable,
...
tablePointer mmu_get_or_create_l2_table(tablePointer masterTable,
...
void mmu_init_process(Process* process) {
	...
	for (t = 0; t < memory_count; t++) {
		...
		mmu_create_direct_mapping_range(process->masterTable,
				m->globalStartAddress, m->userStartAddress, 0);
	}
	...
	for (i = 0; i < device_memory_count; i++) {
		mmu_create_direct_mapping_range(process->masterTable,
				device_memories[i].startAddress,
				device_memories[i].startAddress + device_memories[i].size, 0);
	}

	//
	// map interrupt handlers
	mmu_create_direct_mapping_range(process->masterTable, intvecs_start,
			intvecs_start + intvecs_size, 0);
}
...
void mmu_delete_process_memory(Process* proc) {
	...
	for (masterTableEntry = 0; masterTableEntry < MMU_MASTER_TABLE_PAGE_SIZE;
			masterTableEntry++) {
		// cleanup the l2 table for the masterTableEntry if there is a table
		l2Table =
				(tablePointer) MMU_MASTER_TABLE_ENTRY_TO_L2_ADDRESS((*(masterTable + masterTableEntry)));
		if (l2Table != NULL) {
			// cleanup pages for all l2 entries
			for (l2TableEntry = 0; l2TableEntry < MMU_L2_PAGE_SIZE;
					l2TableEntry++) {
				// cleanup page for l2Entry
				pageAddress =
						MMU_L2_TABLE_ENTRY_TO_PAGE( (*(l2Table+ l2TableEntry)) );
...
uint32_t mmu_virtual_to_physical(tablePointer masterTable,
		uint32_t virtualAddress) {
	// lookup master table entry
	uint32_t masterTableEntry =
			MMU_VIRTUAL_TO_MASTER_TABLE_ENTRY(virtualAddress);
	uint32_t masterTableEntryValue;
	...
	if (masterTableEntry < MMU_MASTER_TABLE_PAGE_SIZE) {
		...
		// is this a section entry?
			...
			return (masterTableEntryValue & 0xFFF00000)
					| (virtualAddress & 0xFFFFF);
		}
	}
